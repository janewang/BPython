...
def loadini(struct, configfile):
    ...
    fill_config_with_default_values(config, {
        'general': {
            ...
            'pastebin_private': False,  # why is pastebin_private set twice, here and two lines down
            'pastebin_url': 'http://bpaste.net/xmlrpc/',
            'pastebin_private': True,    # why is pastebin_private set here again, probably a bug
    ...
    struct.gist_key = config.get('keyboard', 'gist')
    ...
    struct.gist_confirm = config.getboolean('general', 'gist_confirm')
    struct.gist_private = config.getboolean('general', 'gist_private')
    struct.gist_url = config.get('general', 'gist_url')
    struct.gist_private = config.get('general', 'gist_private')
    struct.gist_show_url = config.get('general', 'gist_show_url')
    struct.gist_helper = config.get('general', 'gist_helper')
    ...
    # gist key
    for key in (struct.gist_key, struct.save_key):
        key_dispatch[key]
